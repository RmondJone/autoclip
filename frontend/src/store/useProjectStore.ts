...
export const useProjectStore = create<ProjectStore>((set, get) => ({
    ...
    reorderCollectionClips: async (projectId: string, collectionId: string, newClipIds: string[]) => {
        ...
        // 获取原始状态
        const state = get()
        console.log('Current state projects:', state.projects.map(p => ({
            id: p.id,
            collectionsCount: p.collections?.length || 0
        })))
        console.log('Current state currentProject:', state.currentProject ? {
            id: state.currentProject.id,
            collectionsCount: state.currentProject.collections?.length || 0
        } : null)
        ...
        console.log('Found project:', originalProject ? {
            id: originalProject.id,
            collectionsCount: originalProject.collections?.length || 0
        } : null)

        if (originalProject?.collections) {
            console.log('Project collections:', originalProject.collections.map(c => ({
                id: c.id,
                title: c.collection_title
            })))
        }

        console.log('Found collection:', originalCollection ? {
            id: originalCollection.id,
            title: originalCollection.collection_title
        } : null)
