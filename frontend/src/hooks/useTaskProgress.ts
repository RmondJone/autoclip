...
import {getStageDisplayName} from "@/stores/useSimpleProgressStore.ts";
...
export const useTaskProgress = (options: UseTaskProgressOptions) => {
    ...
    const handleWebSocketMessage = useCallback((message: any) => {
        if (message.type === 'task_progress_update' && message.task_id === taskId) {
            ...
            if (progressMessage.status === 'DONE') {
                ...
                if (progressMessage.status === 'FAIL') {
    ...
    const performFinalStateCheck = useCallback(async () => {
        ...
        try {
            ...
            const response = await projectApi.getProcessingStatus(taskId);

            if (response) {
                const apiState: TaskProgressState = {
                    task_id: taskId,
                    progress: response.progress || 0,
                    step: response.current_step || 0,
                    total: 6,
                    phase: 'unknown',
                    message: getStageDisplayName(`${response.current_step}`) || '未知状态',
